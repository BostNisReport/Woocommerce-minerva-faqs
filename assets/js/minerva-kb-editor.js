...
(function($) {
    ...
    function handleShortcodeInsert(e) {
        ...
        var before = store.parsedShortcode ? '' : '[' + store.shortcode + ']';
        var after = store.parsedShortcode ? '' : '[/' + store.shortcode + ']';
        ...
        if (store.shortcodesCache[store.shortcode].count) { // has options
            ...
            if (store.parsedShortcode) {
                ...
                result = before + (shortcodeInfo.hasContent ? store.selectedText : '') + after;
            }
