	...
	class KST_WP_Import extends WP_Importer {
				...
				if (isset($sections) && !empty($sections)) {
					...
					foreach ($sections as $key => $section) {
						...
						$topics = array_map(function($id) use ($available_topics, &$index) {
							if ($id === 'recent' || $id === 'top_views' || $id === 'top_likes' || in_array($id, $available_topics)) {
								return $id;
							}
		...
		function process_terms() {
			...
			foreach ( $this->terms as $term ) {
				...
				if (MKB_Options::option('disable_faq') && $term['term_taxonomy'] == 'faq_category') {
					continue;
				}
		...
		function process_posts() {
			foreach ( $this->posts as $post ) {
				...
				if (MKB_Options::option('disable_faq') && $post['post_type'] == 'faq') {
					continue;
				}
